...
function Banner({
  ...
  return (
    <div className={`text-sm border rounded p-2 flex items-start justify-between ${color}`}>
...
export default function Home() {
  ...
  const validationMessage = useMemo(() => {
    ...
    if (desc.length > MAX_DESC) return `Description must be ≤ ${MAX_DESC} chars.`;

    if (!Number.isFinite(plusCredits) || !Number.isInteger(plusCredits)) return "Plus credits must be an integer.";
    if (!Number.isFinite(minusCredits) || !Number.isInteger(minusCredits)) return "Minus credits must be an integer.";
    if (plusCredits <= 0) return "Plus credits must be > 0.";
    if (plusCredits > 255 || minusCredits > 255 || minusCredits < 0) return "Credits must be within 0..255.";

    // Rule: if minus is used, require P ≥ 2M
    if (minusCredits > 0 && plusCredits < 2 * minusCredits) return "Invalid credit ratio. Require plus ≥ 2 × minus.";

    if (!Number.isFinite(startInSec) || startInSec < 0) return "Start offset must be ≥ 0 seconds.";
    if (!Number.isFinite(durationSec) || durationSec <= 0) return "Duration must be > 0 seconds.";
  ...
  const voteHref =
    hydrated && wallet?.publicKey ? `/vote/${wallet.publicKey.toBase58()}/${pollId}` : undefined;
      ...
      <section className="space-y-3 border p-4 rounded-lg">
        ...
        <div className="flex flex-wrap gap-2">
          <button
            ...
            className={`px-3 py-2 text-white rounded ${initDisabled ? "bg-gray-300 cursor-not-allowed" : "bg-black"}`}
            disabled={initDisabled}
            title={initDisabled ? validationMessage ?? "" : "Create poll"}
          >
            Create / Re-init
          </button>
          <button onClick={() => refreshPoll(true)} className="px-3 py-2 bg-gray-200 rounded">
            Refresh
          </button>
          {voteHref && (
            <Link href={voteHref} className="px-3 py-2 bg-indigo-600 text-white rounded">
              Open voting page
            </Link>
          )}
        </div>
      ...
      <section className="space-y-3 border p-4 rounded-lg">
        ...
        <div className="grid grid-cols-3 gap-3">
          ...
          <div className="flex items-end">
            <button onClick={addOption} className="px-3 py-2 bg-indigo-600 text-white rounded w-full">
